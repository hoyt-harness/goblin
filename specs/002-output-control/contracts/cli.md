...
Two new top-level fields appear when the corresponding flags are non-default:
...
- `grid_rows`: Written when grid mode is active and `cfg.GridRows > 0`.
  Omitted when grid pages are square (default behavior); consumers should
  treat absent `grid_rows` as equal to `grid_cols`.
